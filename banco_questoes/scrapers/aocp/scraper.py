"""Scraper para certames e provas do Instituto AOCP utilizando a arquitetura Lean (CLAUDE.md 1.2)."""
...
from scrapers.aocp import parser

BASE_URL = "https://www.institutoaocp.org.br/"


class AOCPScraper(BaseBancaScraper):
    """Implementação do coletor do Instituto AOCP compatível com o ciclo Process & Purge."""

    nome_banca: str = "Instituto AOCP"

    def listar_concursos(self) -> list[dict[str, Any]]:
        """Varre os concursos publicados no portal do Instituto AOCP."""
        url = urljoin(BASE_URL, "concursos.jsp")
        try:
            resp = self.sessao.get(url, timeout=20)
            resp.raise_for_status()
            soup = BeautifulSoup(resp.text, "html.parser")
        except Exception as e:
            print(f"Erro ao listar concursos AOCP: {e}")
            return []

        concursos = []
        for link in soup.find_all("a", href=re.compile(r"concurso\.jsp\?id=(\d+)", re.IGNORECASE)):
            href = link.get("href", "")
    ...
    def listar_arquivos(self, slug: str) -> list[dict[str, Any]]:
        ...
        url = urljoin(BASE_URL, f"concurso.jsp?id={slug}")
        try:
            resp = self.sessao.get(url, timeout=20)
            resp.raise_for_status()
            soup = BeautifulSoup(resp.text, "html.parser")
        except Exception as e:
            print(f"Erro ao listar arquivos do certame AOCP {slug}: {e}")
            return []
    ...
    def baixar_arquivo(self, slug: str, nome_arquivo: str) -> bytes:
        ...
        if not url_alvo:
            url_alvo = urljoin(BASE_URL, f"arquivos/{nome_arquivo}")
    ...
    def classificar_arquivo(self, arquivo: dict[str, Any]) -> str:
        ...
        if "CADERNO" in alvo or "PROVA OBJETIVA" in alvo or "CADERNO DE QUESTOES" in alvo:
