...
def test_extrai_blocos_da_pagina_real(html):
    blocos = parser.extrair_blocos(html)
...
def test_extrair_cargo_da_prova_isolada():
    ...
    cargo = parser.extrair_cargo_da_prova(
...
def test_extrair_cargo_da_prova_sem_dados_retorna_none():
    assert parser.extrair_cargo_da_prova(None, "IAN", "2026", "X") is None
    assert parser.extrair_cargo_da_prova("qualquer coisa", None, "2026", "X") is None
...
def test_salvar_pagina_grava_no_banco(html, tmp_path):
    ...
    novas = coletar_qc.salvar_pagina(html, con, "Língua Portuguesa")
    assert novas >= 5
    ...
    assert coletar_qc.salvar_pagina(html, con, "Língua Portuguesa") == 0
...
def test_salvar_pagina_grava_cargo_categoria_tema(html):
    ...
    con = db.conectar()
    coletar_qc.salvar_pagina(html, con, "Contabilidade Geral")
...
def test_atingiu_limite_pelo_atributo_do_botao():
    ...
    assert parser.atingiu_limite(html_limite)
...
def test_atingiu_limite_pelo_modal():
    ...
    assert parser.atingiu_limite(html_modal)
...
def test_nao_atingiu_limite():
    ...
    assert not parser.atingiu_limite(html_normal)
    assert not parser.atingiu_limite("<div>página normal</div>")
...
def test_extrair_resposta_do_bloco_quando_errou():
    ...
    gabarito, comentario = parser.extrair_resposta_do_bloco(bloco_html, "A")
    assert gabarito == "B"
...
def test_extrair_resposta_do_bloco_quando_acertou():
    ...
    gabarito, comentario = parser.extrair_resposta_do_bloco(bloco_html, "b")
    assert gabarito == "B"
    assert comentario is None


def test_extrair_resposta_do_bloco_sem_nada():
    assert parser.extrair_resposta_do_bloco("<div></div>", "A") == (None, None)
...
def test_extrai_texto_associado_e_imagens():
    ...
    blocos = parser.extrair_blocos(FIXTURE_RESPONDIDA.read_text(encoding="utf-8"))
