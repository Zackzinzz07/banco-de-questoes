...
e_prova_cargo = _estilo(
    "ProvaCargo", fontSize=8.5, leading=11, alignment=TA_CENTER, textColor=PRETO
)
e_aviso = _estilo("Aviso", fontSize=7.5, leading=9.5, alignment=TA_JUSTIFY, textColor=CINZA)
e_instrucoes = _estilo(
    "Instrucoes", fontSize=8, leading=10.5, alignment=TA_JUSTIFY, textColor=PRETO
)
e_secao = _estilo(
    "Secao",
    fontName="Helvetica-Bold",
...
e_alt = _estilo(
    "Alt", fontSize=9.2, leading=11.2, alignment=TA_JUSTIFY, textColor=PRETO, spaceAfter=1
)
e_alt.leftIndent = 12
e_alt.firstLineIndent = -12  # recuo pendurado: "(A) " fica pra fora
e_texto_base = _estilo(
    "TextoBase", fontSize=9.2, leading=11.2, alignment=TA_JUSTIFY, textColor=PRETO, spaceAfter=4
)
e_chamada_texto = _estilo(
    "ChamadaTexto",
    fontName="Helvetica-Bold",
    fontSize=9.2,
    leading=11.2,
...
e_origem = _estilo(
    "Origem", fontName="Helvetica-Oblique", fontSize=7, leading=9, textColor=CINZA, spaceAfter=1
)
e_gab = _estilo(
    "Gab", fontSize=8.5, leading=10.5, alignment=TA_JUSTIFY, textColor=PRETO, spaceAfter=5
)
...
def _origem(q):
    partes = [
        q.get("banca"),
        q.get("orgao"),
        str(q["ano"]) if q.get("ano") else None,
        q.get("assunto"),
    ]
...
def _cabecalho(materia, quantidade, concurso=None):
    ...
    return linhas + [
        ...
        Paragraph(
            "Cada questão possui até 5 (cinco) opções de resposta e apenas "
        ...
        Paragraph("O gabarito comentado encontra-se ao final do caderno.", e_instrucoes),
    ]
...
def _questao_flowables(numero, q):
    ...
    partes.append(Paragraph(f"<b>QUESTÃO {numero}. {escape(q['enunciado'])}</b>", e_questao))
    for letra in sorted(q["alternativas"]):
        partes.append(Paragraph(f"<b>({letra})</b> {escape(q['alternativas'][letra])}", e_alt))
    partes.append(Spacer(1, 4))
    return partes
...
def gerar(materia, quantidade, arquivo_saida=None, con=None, concurso=None):
    ...
    if arquivo_saida is None:
        ...
        arquivo_saida = (
            Path(__file__).resolve().parent / f"simulado_{nome}_{date.today():%Y%m%d}.pdf"
        )
...
def gerar_completo(quantidade, arquivo_saida=None, con=None, concurso=None):
    ...
    if arquivo_saida is None:
        arquivo_saida = (
            Path(__file__).resolve().parent / f"simulado_geral_{date.today():%Y%m%d}.pdf"
        )
    arquivo_saida = Path(arquivo_saida)
